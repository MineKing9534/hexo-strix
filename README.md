...
| Path | What it is |
|------|-----------|
| [`hexo-rs/`](hexo-rs/README.md) | Rust workspace, six crates: the game engine (`hexo-engine`); Gumbel MCTS + graph builders (`hexo-mcts`); the fully-forcing VCF solver + deep prover drivers (`hexo-solver`, no NN/MCTS/PyO3 deps — re-exported by `hexo-mcts`); a pure-Rust GNN forward pass with no libtorch/Python (`hexo-infer`, powers the browser bot, a native serving fast-path, and a native inference server binary); a browser/WebWorker bot (`hexo-wasm`, which also exports a standalone `StrixSolver`); and the crate maturin actually builds into the `hexo_rs` Python module (`hexo-py`, which re-registers `hexo-mcts`'s bindings and adds `hexo-infer`-backed native inference). |
