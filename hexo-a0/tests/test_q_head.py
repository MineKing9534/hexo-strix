...
class TestTrainerQLoss:
    ...
    def test_q_diagnostics_surface_when_enabled(self):
        from hexo_a0.trainer import _forward_and_loss
        net = HeXONet(_cfg(q_head=True))
        examples = [
            _example9(value=1.0, q=[0.9, 0.8, -0.1, 0.0], visits=[10, 5, 3, 1]),
        ...
        out = _forward_and_loss(net, examples, torch.device("cpu"), q_loss_weight=0.5)
        # MAE is the sqrt-ish companion to MSE: q_mae^2 <= q_loss roughly, and
        # it's positive whenever the head isn't perfectly predicting.
        assert torch.is_tensor(out["q_mae"])
        assert out["q_mae"].item() > 0.0
        # Coverage: all legal moves visited in both examples → 1.0.
        assert out["q_coverage"].item() == pytest.approx(1.0, abs=1e-6)
        # Population sums [n, Σx, Σy, Σx², Σy², Σxy] over visited moves.
        assert torch.is_tensor(out["q_corr_sums"])
    ...
    def test_q_coverage_reflects_unvisited_moves(self):
        from hexo_a0.trainer import _forward_and_loss
        net = HeXONet(_cfg(q_head=True))
        # 4 legal moves, only 1 visited → coverage 0.25.
        examples = [_example9(num_legal=4, value=1.0, q=[0.5, 0.0, 0.0, 0.0],
                              visits=[1, 0, 0, 0])]
        out = _forward_and_loss(net, examples, torch.device("cpu"), q_loss_weight=1.0)
        assert out["q_coverage"].item() == pytest.approx(0.25, abs=1e-6)

    def test_q_diagnostics_zero_when_weight_off(self):
        from hexo_a0.trainer import _forward_and_loss
        net = HeXONet(_cfg(q_head=True))
        examples = [_example9(value=1.0)]
        out = _forward_and_loss(net, examples, torch.device("cpu"), q_loss_weight=0.0)
        assert out["q_mae"] == 0.0
        assert out["q_coverage"] == 0.0
        assert out["q_corr_sums"] is None
